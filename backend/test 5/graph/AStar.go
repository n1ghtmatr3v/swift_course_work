...
func (g *Graph) AStar(start uint32, goal uint32, startVirtual VirtualVertex, endVirtual VirtualVertex) AStarResult {
	...
	start_f := g.Heuristic(start, goal, startVirtual, endVirtual)
	...
	for !min_heap.isEmpty() {
		...
		allNeighbors := g.GetAllNeighbors(v, startVirtual, endVirtual)

		for i := 0; i < len(allNeighbors); i++ {
			to := allNeighbors[i].Vertex
			w := allNeighbors[i].Weight
			...
			if newDist < dist[to] {
				...
				newF := newDist + g.Heuristic(to, goal, startVirtual, endVirtual)
